...
constexpr auto ATTRIBUTES =
    makeAttributes<2>(makeAttributeData, {IN_POSITION_STR, IN_UV_STR});
}  // namespace
...
void SpriteBatch::draw(const std::shared_ptr<Texture2D>& texture,
    ...
    model = glm::translate(
        model, glm::vec3(-origin.x * size.x, -origin.y * size.y, 0.0f));

    // S
    model = glm::scale(model, glm::vec3(size, 1.0f));

    auto uvModel = glm::translate(glm::mat4(1.0f),
                                  glm::vec3(sourceRect.x, sourceRect.y, 0.0f));
...
void SpriteBatch::flush() {
    ...
    auto renderer = m_rendererPtr.lock();
    // AlphaBlend
    renderer->enableBlend(Renderer::BlendParam::SRC_ALPHA,
                          Renderer::BlendParam::ONE_MINUS_SRC_ALPHA);

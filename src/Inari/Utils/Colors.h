...
namespace inari::colors {
    ...
    template <int size>
    inline constexpr glm::vec<size, float, glm::defaultp> toGL(const std::string_view& hexValue)
    {
        assert(!hexValue.empty() && "Color string is empty");
        assert(hexValue.front() == '#' && "Color string is not started with hash");
        assert(hexValue.size() - 1 == size * 2 && "The size of the color string doesn't match the requested size");

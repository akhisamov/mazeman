...
namespace {
inari::Tileset createTileset(cute_tiled_tileset_t* cuteTileset) {
    inari::Tileset tileset;
    if (cuteTileset) {
        ...
        tileset.size =
            glm::vec2(cuteTileset->tilewidth, cuteTileset->tileheight);
    }
    return tileset;
}
}  // namespace
...
const std::vector<Tileset>& LevelMap::getTilesets() const {
    return m_tilesets;
}
